					...
					<table class="mdl-data-table mdl-js-data-table mdl-shadow--2dp">
						...
						<tbody>
							<?php
								...
								$query = "SELECT COUNT(*) FROM inspection_template WHERE CONCAT(model, ' ', partnumber, ' ', project, ' ', customer, ' ', status) LIKE '%$keyword%' LIMIT $result_per_page";
								$statement1 = $connect->prepare($query);
								$statement1->execute();
								$total_rows = $statement1->fetchColumn();
								$total_pages = ceil($total_rows / $result_per_page);

								$query = "SELECT * FROM inspection_template WHERE CONCAT(model, ' ', partnumber, ' ', project, ' ', customer, ' ', status) LIKE '%$keyword%' LIMIT $offset, $result_per_page";
							...
							<tr class="pagination">							
								<td>
									<div>
										...
										<button class="mdl-button mdl-js-button mdl-button--icon" onclick="window.location.href='<?= HOST_URL . '/inspection-template' . '/search/?q=' . $keyword . '&pagenum=' ?><?= $pagenum - 1 ?>'" <?= $pagenum <= 1 || $pagenum > $total_pages ? 'disabled' : '' ?>>
											<i class="material-icons">keyboard_arrow_left</i>
										</button>
										<button class="mdl-button mdl-js-button mdl-button--icon" onclick="window.location.href='<?= HOST_URL . '/inspection-template' . '/search/?q=' . $keyword . '&pagenum=' ?><?= $pagenum + 1 ?>'" <?= $pagenum >= $total_pages ? 'disabled' : '' ?>>
